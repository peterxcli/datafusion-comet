...
impl PreparedAccessPlan {
    ...
    fn new(row_group_indexes: Vec<usize>, row_selection: Option<RowSelection>) -> Result<Self> {
        Ok(Self {
            row_group_indexes,
            row_selection,
        })
...
mod test {
    ...
    fn test_invalid_too_few() {
        let access_plan = ParquetAccessPlan::new(vec![
            ...
            RowGroupAccess::Selection(vec![RowSelector::select(5), RowSelector::skip(7)].into()),
            RowGroupAccess::Scan,
            RowGroupAccess::Scan,
        ]);
    ...
    fn parquet_metadata_with_int_mins(mins: &[i32]) -> ParquetMetaData {
        ...
        let row_groups: Vec<RowGroupMetaData> = mins
            .iter()
            .map(|&m| {
                let stats = ParquetStatistics::int32(Some(m), Some(m), None, Some(0), false);
                let column = ColumnChunkMetaData::builder(schema_descr.column(0))
            ...
            .collect();
        let file_metadata = FileMetaData::new(0, 0, None, None, schema_descr.clone(), None);
    ...
    fn reorder_by_statistics_sorts_row_groups_asc_by_min() {
        ...
        let result = plan
            .reorder_by_statistics(&lex_ordering_a_asc(), &metadata, &arrow_schema_a_int())
            .unwrap();
    ...
    fn reorder_by_statistics_breaks_leading_ties_with_secondary_column() {
        let metadata = parquet_metadata_with_two_col_mins(&[(1, 300), (1, 100), (1, 200)]);
        let plan = PreparedAccessPlan::new(vec![0, 1, 2], None).unwrap();
        let order =
            LexOrdering::new(vec![sort_expr("a", 0, false), sort_expr("b", 1, false)]).unwrap();
    ...
    fn reorder_by_statistics_honors_secondary_direction() {
        let metadata = parquet_metadata_with_two_col_mins(&[(1, 100), (1, 300), (0, 500)]);
        let plan = PreparedAccessPlan::new(vec![0, 1, 2], None).unwrap();
        let order =
            LexOrdering::new(vec![sort_expr("a", 0, false), sort_expr("b", 1, true)]).unwrap();
    ...
    fn reorder_by_statistics_normalizes_desc_desc_for_reverse() {
        let metadata = parquet_metadata_with_two_col_mins(&[(1, 300), (2, 100), (1, 100)]);
        let plan = PreparedAccessPlan::new(vec![0, 1, 2], None).unwrap();
        let order =
            LexOrdering::new(vec![sort_expr("a", 0, true), sort_expr("b", 1, true)]).unwrap();
    ...
    fn reorder_by_statistics_keeps_leading_prefix_on_non_column_secondary() {
        let metadata = parquet_metadata_with_two_col_mins(&[(5, 300), (3, 100), (4, 200)]);
        let plan = PreparedAccessPlan::new(vec![0, 1, 2], None).unwrap();
