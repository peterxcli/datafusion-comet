...
impl TreeNodeVisitor<'_> for PushdownChecker<'_> {
    ...
    fn f_down(&mut self, node: &Self::Node) -> Result<TreeNodeRecursion> {
        ...
        if let Some(func) = ScalarFunctionExpr::try_downcast_func::<GetFieldFunc>(node.as_ref()) {
            ...
            if let Some(column) = args.first().and_then(|a| a.downcast_ref::<Column>()) {
                ...
                let is_map_column = self
                    ...
                    .ok()
                    .map(|idx| {
                        matches!(self.file_schema.field(idx).data_type(), DataType::Map(_, _))
                    })
                    .unwrap_or(false);
        ...
        if ScalarFunctionExpr::try_downcast_func::<InputFileNameFunc>(node.as_ref()).is_some()
            || ScalarFunctionExpr::try_downcast_func::<FileRowIndexFunc>(node.as_ref()).is_some()
        {
...
///
/// Unlike DataFusion's predicate read-plan builder (which is used for
...
fn build_read_plan_with_cast_clipping(
    ...
    let get_field_accesses: Vec<StructFieldAccess> = struct_accesses
        ...
        .filter(|a| {
            ...
            !whole_roots.contains(&a.root_index) && !fallback_roots.contains(&a.root_index)
        })
        .cloned()
        .collect();
    ...
    ParquetReadPlan {
        projection_mask: ProjectionMask::leaves(schema_descr, leaf_indices.iter().copied()),
...
fn leaves_grouped_by_root(schema_descr: &SchemaDescriptor) -> BTreeMap<usize, Vec<usize>> {
...
pub(crate) fn assemble_read_plan(
    ...
    let mut leaf_indices = leaf_indices_for_roots(root_indices.iter().copied(), schema_descr);
    leaf_indices.extend_from_slice(&resolve_struct_field_leaves(&access_tree, schema_descr));
    leaf_indices.sort_unstable();
    leaf_indices.dedup();

    let projection_mask = ProjectionMask::leaves(schema_descr, leaf_indices.iter().copied());
...
fn root_level_plan(
    ...
    let projection_mask = ProjectionMask::roots(schema_descr, root_indices.iter().copied());
...
fn leaf_indices_for_roots<I>(root_indices: I, schema_descr: &SchemaDescriptor) -> Vec<usize>
where
