...
use datafusion_physical_plan::filter_pushdown::{FilterPushdownPropagation, PushedDownPredicate};
...
pub(crate) fn parse_coerce_int96_string(str_setting: &str) -> datafusion_common::Result<TimeUnit> {
...
pub(crate) fn parse_coerce_int96_tz_string(tz: &str) -> datafusion_common::Result<Arc<str>> {
    tz.parse::<Tz>().map_err(|e| {
        DataFusionError::Configuration(format!("Invalid parquet coerce_int96_tz {tz:?}: {e}"))
    })?;
    Ok(Arc::<str>::from(tz))
}
...
impl FileSource for ParquetSource {
    ...
    fn create_morselizer(
        ...
        if coerce_int96_tz.is_some() && coerce_int96.is_none() {
            warn!("coerce_int96_tz is set but coerce_int96 is not; the timezone will be ignored");
        }
    ...
    fn try_pushdown_projection(
        ...
        let (table_schema, row_index_col) = table_schema_with_row_index_col(self.table_schema());

        source.table_schema = table_schema;
        source.projection =
            rewrite_file_row_index_projection(&self.projection, projection, &row_index_col)?;
    ...
    fn try_pushdown_sort(
        ...
        let reversed_satisfies = reversed_eq_properties.ordering_satisfy(order.iter().cloned())?;
...
fn table_schema_with_row_index_col(table_schema: &TableSchema) -> (TableSchema, Column) {
    ...
    {
        let virtual_offset =
            table_schema.file_schema().fields().len() + table_schema.table_partition_cols().len();
    ...
    let row_index_field =
        Arc::new(Field::new(&row_index_name, DataType::Int64, true).with_extension_type(RowNumber));
    (
