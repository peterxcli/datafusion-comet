        ...
        (DataType::Struct(p_children), DataType::Struct(t_children)) => {
            ...
            let kept_children: Fields = p_children
                ...
                .filter_map(|pc| {
                    ...
                    let before = kept.len();
                    let pruned =
                        clip_type(pc.data_type(), tc.data_type(), next_leaf, kept, unclippable);
                    if kept.len() == before {
...
fn keep_all_leaves(dt: &DataType, next_leaf: &mut usize, kept: &mut Vec<usize>) -> DataType {
...
mod tests {
    ...
    fn clip_keeps_physical_leaf_types() {
        let physical = struct_of(vec![Field::new("x", DataType::Int32, true), utf8("pad")]);
    ...
    fn no_clip_on_map() {
        ...
        let physical = DataType::Map(entries(vec![utf8("key"), int64("a"), int64("b")]), false);
    ...
    fn reader_drops_struct_child_with_no_selected_leaves() {
        ...
        let batch = RecordBatch::try_new(Arc::clone(&schema), vec![Arc::new(outer)]).unwrap();
    ...
    fn arrow_reader_emits_clipped_type_for_masked_list_struct() {
        ...
        let batch = RecordBatch::try_new(Arc::clone(&schema), vec![Arc::new(events)]).unwrap();
