...
impl PagePruningResult {
    fn new(access_plan: ParquetAccessPlan, pages_skipped_by_fully_matched: usize) -> Self {
        Self {
            access_plan,
...
impl PagePruningAccessPlanFilter {
    ...
    pub(crate) fn prune_plan_with_page_index_and_metrics(
        ...
        if parquet_metadata.offset_index().is_none() || parquet_metadata.column_index().is_none() {
            debug!(
        ...
        for row_group_index in row_group_indexes {
            ...
            if access_plan.is_fully_matched(row_group_index) {
                let page_count = fully_matched_page_count(row_group_index, parquet_metadata);
            ...
            for predicate in page_index_predicates {
                ...
                let converter =
                    StatisticsConverter::try_new(column.name(), arrow_schema, parquet_schema);
            ...
            if let Some(overall_selection) = overall_selection {
                ...
                total_select += parquet_metadata.row_group(row_group_index).num_rows() as usize;
            }
...
fn fully_matched_page_count(row_group_index: usize, parquet_metadata: &ParquetMetaData) -> usize {
...
fn prune_pages_in_one_row_group(
    ...
    let Some(page_row_counts) = pruning_stats.page_row_counts() else {
        debug!("Can not determine page row counts for row group {row_group_index}, skipping");
        metrics.predicate_evaluation_errors.add(1);
        return None;
    };
...
impl<'a> PagesPruningStatistics<'a> {
    ...
    fn try_new(
        ...
        let Some(offset_index_metadata) = row_group_page_offsets.get(parquet_column_index) else {
            trace!(
